                ...
                if dev_loader is not None and self.args.local_rank == 0:
                    if global_step % eval_step == 0:
                        ...
                        if macro_f1 > best_dev_micro_f1:
                            ...
                            save_path = os.path.join(self.args.output_dir, self.args.data_name)
...
def main_worker(local_rank, args):
    ...
    if sys.platform == "win32":
        ...
        print(init_method)
        dist.init_process_group(backend="gloo", init_method=init_method, rank=local_rank,
    ...
    if args.do_train:
        if args.retrain:
            ...
            if args.local_rank == 0:
                print("========加载模型继续训练========")
            # 训练和验证
            trainer.train(r_model, train_loader, train_sampler, dev_loader=None)
        else:
            if args.local_rank == 0:
                print('========进行训练========')
            model.cuda(args.local_rank)
